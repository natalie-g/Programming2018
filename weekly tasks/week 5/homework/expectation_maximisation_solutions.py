...
class EM(object):
    ...
    def e_step(self, observation):
        ...
        '''
        total = float("-inf")
        mixture_unnormalized = {}
        for i in range(self.num_components):
            log_value = self.mixture_weights[i]+self.component_distributions[i].log_prob(observation)
            total = log_add(log_value, total)
            mixture_unnormalized[i] = log_value

        #normalize and update
        for i in range(self.num_components):
            normalized = mixture_unnormalized[i] - total
            self.expected_component_counts[i] = log_add(normalized, self.expected_component_counts[i])
            if observation:
                self.expected_observation_counts[i] = log_add(log(observation) + normalized, self.expected_observation_counts[i])
