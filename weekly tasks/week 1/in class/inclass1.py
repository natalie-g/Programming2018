...
#1. Assign values 5 and 3 to variables named first and second; print the values of the variables, one per line (to help you get going, this question is already answered)
...
#8. Let us encapsulate the behavior in the last piece of code. Write a function called two_letter_sample that will take any string as its argument and return a string with two randomly sampled letters if the original string is at least of length two. Check that the function gives a sensible output using a few strings as an example.

#9. Create a second function, called check_if_vowels. The function will take a (two-letter) string and will check that the string is either 'ae' or 'ea'. If this is so, the function returns True, otherwise it returns False. Test that the function works.
...
#Note: this is a recursive function and some care has to be taken here. It usually helps to add print statemements to the function. For your convenience, parts of the function are already in place.
...
def recursive_call(input_string, repetitions, successes):
    ...
    if repetitions > 0:
        pass
        #write your code instead of pass; in your code, you should sampe from input_string; then if the sample is 'ae' or 'ea', the count in successes should be increased by 1; after that, recursive_call should be called again
    return successes
...
#12. The number will vary, but it should be close (up to 1 percent) to the probability arrived at analytically. Write the formula, compare the two values
